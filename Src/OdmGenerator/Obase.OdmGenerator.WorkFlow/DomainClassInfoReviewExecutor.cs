...
///     领域类信息审核执行器
/// </summary>
public class DomainClassInfoReviewExecutor : Executor<List<DomainClassExtractResult>, DomainClassReviewResult>
{
    /// <summary>
    ///     领域类信息审核代理
    /// </summary>
    private readonly DomainClassInfoReviewAgent _agent;
    ...
    public DomainClassInfoReviewExecutor(List<DomainClassInfo> domainClassInfos, IApikeyConfiguration config,
        string knowledge = null) :
        base(nameof(DomainClassInfoReviewExecutor))
    {
        ...
        //创建领域类预处理器代理
        _agent = new DomainClassInfoReviewAgent(domainClassInfos, config, knowledge, output.ToString());
    }

    /// <summary>
    ///     所使用的领域类信息审核代理
    /// </summary>
    public DomainClassInfoReviewAgent Agent => _agent;

    /// <summary>
    ///     处理方法
    /// </summary>
    /// <param name="input">领域信息审核输入</param>
    /// <param name="context">工作流上下文</param>
    /// <param name="cancellationToken">取消Token</param>
    /// <returns>处理结果</returns>
    public override async ValueTask<DomainClassReviewResult> HandleAsync(List<DomainClassExtractResult> input,
        IWorkflowContext context,
        CancellationToken cancellationToken = new())
    {
        //发布事件
        await context.AddEventAsync(new ObjectDataModelGenerateEvent("领域信息抽取审核开始执行."), cancellationToken);

        var question = new StringBuilder("请对以下领域类型分析结果进行审核,并给出审核意见:").AppendLine();
        ...
        //处理Json字符串
        var json = AgentOutputHelper.DeserializeJsonObject<DomainClassReviewResult>(result.Text);

        //通过
        if (json.IsValid)
            //通过了 进入输出配置流程
            json.DomainClassExtractResults = input;

        //发布事件
        await context.AddEventAsync(new ObjectDataModelGenerateEvent("领域信息抽取审核执行完成."), cancellationToken);
